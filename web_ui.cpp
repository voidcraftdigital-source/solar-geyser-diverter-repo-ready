...
namespace {
  ...
  String pageShell(const String &title, const String &body){
    ...
    html += body;
    html += "<footer>Rev A4 Geyser Diverter &mdash; firmware V0.10</footer>";
    html += "</body></html>";
    return html;
  }

  String cloudStatusText(){
    if (!CloudSync::hasCredentials()) return "not configured";
    if (!CloudSync::everSynced()) return "waiting for first sync…";
    if (CloudSync::lastSyncOk()) return "OK, " + String(CloudSync::lastSyncAgoMs() / 1000) + "s ago";
    return "failing: " + CloudSync::lastError();
  }
  ...
  void handleRoot(){
    ...
    body += "<tr><td class='k'>Cloud sync</td><td class='v'>" + cloudStatusText() + "</td></tr>";
    body += "</table>";
    ...
    body += "<a class='btn' href='/cloud'>Cloud sync</a>";
  ...
  String cloudBody(const String &note){
    String body = "<h1>Cloud sync</h1>";
    body += "<p class='tag'>Specification &sect;13 &mdash; V0.10</p>";
    body += "<div class='warn'>There is no factory provisioning tool yet. Get a real device ID/secret by "
            "calling the backend's manufacturing endpoint yourself (<code>POST /api/manufacturing/devices</code> "
            "with your <code>X-Manufacturing-Key</code>), then enter what it returns below. This is the "
            "device's own permanent credential &mdash; not the printed claim code a customer enters when "
            "pairing, which is a separate, single-use value the backend issues at the same time.</div>";
    body += "<table><tr><td class='k'>Status</td><td class='v'>" + cloudStatusText() + "</td></tr></table>";
    if (note.length()) body += "<p class='tag'>" + note + "</p>";
    body += "<form method='POST' action='/cloud/save'>";
    body += "<label>Backend URL</label>";
    body += "<input name='url' value='" + htmlEscape(Settings::cloudBackendUrl()) + "' placeholder='https://your-backend.onrender.com' required>";
    body += "<label>Device ID</label>";
    body += "<input name='id' value='" + htmlEscape(Settings::cloudDeviceId()) + "' placeholder='GD-XXXX-XXXX' required>";
    body += "<label>Device secret</label>";
    body += "<input name='secret' type='password' value='" + htmlEscape(Settings::cloudDeviceSecret()) + "' required>";
    body += "<button type='submit'>Save</button>";
    body += "</form>";
    body += "<form method='POST' action='/cloud/reset' onsubmit=\"return confirm('Clear saved cloud credentials?')\">";
    body += "<button type='submit' style='background:#d03b3b'>Clear</button>";
    body += "</form>";
    body += "<p><a href='/'>&larr; back</a></p>";
    return body;
  }
  ...
  void handleCloudPage(){
    httpServer.send(200, "text/html", pageShell("Cloud sync", cloudBody("")));
  }

  void handleCloudSave(){
    if (!httpServer.hasArg("url") || !httpServer.hasArg("id") || !httpServer.hasArg("secret") ||
        httpServer.arg("url").length() == 0 || httpServer.arg("id").length() == 0 || httpServer.arg("secret").length() == 0){
      httpServer.send(400, "text/plain", "Backend URL, device ID, and device secret are all required");
      return;
    }
    String url = httpServer.arg("url");
    while (url.endsWith("/")) url.remove(url.length() - 1); // avoid a doubled slash against "/api/..."
    Settings::setCloudCredentials(url, httpServer.arg("id"), httpServer.arg("secret"));
    httpServer.send(200, "text/html", pageShell("Cloud sync", cloudBody("Saved. The next sync attempt will use these credentials within a few seconds.")));
  }

  void handleCloudReset(){
    Settings::clearCloudCredentials();
    httpServer.send(200, "text/html", pageShell("Cloud sync", cloudBody("Cleared.")));
  }
...
void WebUI::begin(){
  ...
  httpServer.on("/cloud", HTTP_GET, handleCloudPage);
  httpServer.on("/cloud/save", HTTP_POST, handleCloudSave);
  httpServer.on("/cloud/reset", HTTP_POST, handleCloudReset);
